...
export const approveCreatorAccess = async ({
  requestId,
}: {
  requestId: string;
}) => {
  ...
  const updatedRequest = await db.creatorAccessRequest.update({
    ...
    data: {
      status: CREATOR_ACCESS_REQUEST_STATUS.APPROVED,
    },
  });
  ...
  // Update the user role in the db (only if he is not an admin or a creator)
  if (requestedUser?.role === USER_ROLE.USER) {
  ...
  return {
    success: {
      message: "Creator access request approved!",
      updatedRequest,
    },
  };
};
